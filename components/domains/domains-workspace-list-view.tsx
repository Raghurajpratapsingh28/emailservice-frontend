  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6 max-w-[1200px] mx-auto select-none">
      <div>
        ...
        <h1 className="text-3xl font-extrabold tracking-tight text-[#FFFFFF] mt-1">Domains</h1>
        <p className="text-xs text-[#8A8D96] mt-1.5">Select a workspace to manage its sending domains.</p>
      </div>
      <div className="grid gap-4">
        {workspaces.map(({ workspace, role }) => (
          <button key={workspace.id} onClick={() => handleOpen(workspace.id)} className="w-full text-left p-6 enterprise-card hover:border-[#8A8D96] transition-all group cursor-pointer rounded-[16px]">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-[12px] bg-[#3CD3AD]/10 border border-[#3CD3AD]/25 flex items-center justify-center shrink-0">
                  <Globe className="w-5 h-5 text-[#3CD3AD]" />
                </div>
                <div>
                  <div className="flex items-center gap-2.5 flex-wrap">
                    <span className="text-base font-bold text-[#FFFFFF]">{workspace.name}</span>
                    <span className="text-[9px] font-medium px-2 py-0.5 rounded-[8px] bg-[#8A8D96]/10 border border-[#8A8D96]/20 text-[#8A8D96]">{role}</span>
                    {workspace.status === "inactive" && <span className="text-[9px] font-medium px-2 py-0.5 rounded-[8px] bg-[#FFB020]/10 border border-[#FFB020]/20 text-[#FFB020]">inactive</span>}
                  </div>
