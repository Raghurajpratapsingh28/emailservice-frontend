...
export default function HomeDashboard({ workspaceId }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-6 max-w-[1500px] mx-auto select-none">
      <div className="flex items-center justify-between">
        <div>
          <button onClick={() => router.push("/home")} className="flex items-center gap-1.5 text-[10px] font-mono text-[#7A8499] hover:text-[#B0B8C8] transition-colors mb-2 cursor-pointer">
        ...
        <button onClick={() => load(true)} className="flex items-center gap-2 px-3.5 py-2 bg-[#111319] hover:bg-[#1C1F2D] border border-[#1E2230] rounded-xl text-xs font-semibold text-[#B0B8C8] hover:text-white transition-all cursor-pointer">
          <RefreshCw className={`w-3.5 h-3.5 ${isRefreshing ? "animate-spin text-[#6B7280]" : ""}`} /> Sync Data
        </button>
      </div>
